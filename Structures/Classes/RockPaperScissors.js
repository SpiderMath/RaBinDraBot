	...
	async run() {
		if(!this.challenged.bot) {
			...
			try {
				...
				this.message.channel.send(`${this.challenger} I have DMed you, please respond to move on with the game!`);

				const p1Stuff = await p1Msg.channel.awaitMessages(PMsg => PMsg.author.id === this.challenger.id && rps.includes(PMsg.content.toLowerCase()), { max: 1, time: toMs(60) });
				...
				this.message.channel.send(`${this.challenged} I have DMed you, please respond to move on with the game!`);

				const p2Stuff = await p2Msg.channel.awaitMessages(PMsg => PMsg.author.id === this.challenged.id && rps.includes(PMsg.content.toLowerCase()), { max: 1, time: toMs(60) });
